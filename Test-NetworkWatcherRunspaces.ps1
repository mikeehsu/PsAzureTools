...
class VmIpMap {
    ...
    VmIpMap() {
        ...
        $vms = Get-AzVM | Select-Property -Property Id, 
        foreach $vm in $
...
$pool = [RunspaceFactory]::CreateRunspacePool(1, [int] $MaxConcurrentTests)
...
while ($testLeft -gt 0) {
    ...
    # submit more tests
    $slotsLeft = $MaxConcurrentTests - @($testCases | Where-Object { $_.Runspace }).Count
    foreach ($testCase in $testCases) {
        if ($slotsLeft -le 0) {
            break
        }

        if ($testCase.Result -or $testCase.Runspace) {
            continue
        }
        ...
        if ($testCase.Protocol -eq 'ICMP') {
            $code = {
                ...
                $protocolConfig.Protocol = 'ICMP'
    

                Test-AzNetworkWatcherConnectivity -ResourceGroupName $ResourceGroupName -NetworkWatcherName $NetworkWatcherName `
                    -SourceId $SourceId `
            ...
            $null = $runspace.AddScript($code)
            $null = $runspace.AddParameter('ResourceGroupName', $ResourceGroupName)
            $null = $runspace.AddParameter('NetworkWatcherName', $NetworkWatcherName)
            $null = $runspace.AddParameter('SourceId', $testCase.SourceId)
            $null = $runspace.AddParameter('DestinationAddress', $testCase.DestinationAddress)
            ...
            $null = $runspace.AddParameter('NetworkWatcherName', $NetworkWatcherName)
            $null = $runspace.AddParameter('SourceId', $testCase.SourceId)
            $null = $runspace.AddParameter('SourcePort', $testCase.SourcePort)
            $null = $runspace.AddParameter('DestinationAddress', $testCase.DestinationAddress)
            $null = $runspace.AddParameter('DestinationPort', $testCase.DestinationPort)
        }

        $runspace.RunspacePool = $pool

        $testCase.Runspace = $runspace
        $testCase.RunspaceStatus = $runspace.BeginInvoke()

        if ($testCase.RetryCount -gt 0) {
            Write-Verbose "Retry ($($testCase.RetryCount)) - $($testCase.Protocol)/$($testCase.SourceAddress):$($testCase.SourcePort) -> $($testCase.DestinationAddress):$($testCase.DestinationPort)" 
        }
        else {
            Write-Verbose "Submitted - $($testCase.Protocol)/$($testCase.SourceAddress):$($testCase.SourcePort) -> $($testCase.DestinationAddress):$($testCase.DestinationPort)" 
        }
    ...
    try {
        Write-Verbose "Waiting on job(s) to complete...($($stopwatch.Elapsed))"
        Start-Sleep -Seconds 5
        $completedCases = @($testCases | Where-Object { $_.Runspace -ne $null -and $_.RunspaceStatus.IsCompleted })
        if (-not $completedCases) {
            continue
        }        
    }
    catch {
    ...
    foreach ($testCase in $completedCases) {
        $result = $testCase.Runspace.EndInvoke($testCase.RunspaceStatus)
        if ($result) {
            ...
            $testCase.RetryCount++
            Write-Host "Retry ($($testCase.RetryCount)) - $($testCase.Protocol)/$($testCase.SourceAddress):$($testCase.SourcePort) -> $($testCase.DestinationAddress):$($testCase.DestinationPort)" -ForegroundColor Red
        }
 
        # $testCase.Runspace.Dispose()
        $testCase.Runspace = $null
        $testCase.RunspaceStatus = $null
    }
