...
function InvokeRunCommand {
    ...
    if ($WhatIf) {
        Write-Output "WhatIf: $($ResourceGroupName)/$($Name) Shutdown would be executed."
        return $true
    }
...
function ExecuteAction {
    ...
    if (-not $vm) {
        ...
        Write-Output "Script complete. ($(New-TimeSpan $StartTime (Get-Date).ToString()) elapsed)"
        return
    }

    try {
        if ($Action -eq 'Start') {
            Write-Output "$($vm.ResourceGroupName)/$($vm.Name) starting..." 
            $result = $vm | Start-AzVM -WhatIf:$WhatIf -NoWait

        }
        elseif ($Action -eq 'Stop' -or $Action -eq 'Deallocate') {
            Write-Output "$($vm.ResourceGroupName)/$($vm.Name) stopping..." 
            $result = $vm | Stop-AzVM -Force -WhatIf:$WhatIf -NoWait
        }
        elseif ($Action -eq 'Shutdown') {
            if ($vm.StorageProfile.OsDisk.OsType -eq 'Windows') {
                Write-Output "$($vm.ResourceGroupName)/$($vm.Name) (windows) shutting down ..." 
            ...
            elseif ($vm.StorageProfile.OsDisk.OsType -eq 'Linux') {
                Write-Output "$($vm.ResourceGroupName)/$($vm.Name) (linux) shutting down..." 
        ...
        if ($result) {
            if ($result -is [boolean] -or $result.IsSuccessStatusCode) {
                ...
                Write-Output "...Failed. $($result.StatusCode - $result.ReasonPhrase)"
            }
        }
        else {
            if ($WhatIf) {
...
if ($IncludeResourceGroupNames) {
    ...
    Write-Output 'IncludeResourceGroupNames: ' $($IncludeResourceGroupNames -join ',')
}

if ($IncludeVmNames) {
    [string[]] $IncludeVmNames = ($IncludeVmNames -Split ',').Trim().Replace('"', '').Replace("'", '')
    Write-Output 'IncludeVmNames:' $($IncludeVmNames -join ',')
}

if ($IncludeTags) {
    Write-Output "IncludeTags: $IncludeTags"
...
if ($IncludeVmState) {
    ...
    Write-Output "IncludeVmState: $IncludeVmState"
}

if ($ExcludeResourceGroupNames) {
    [string[]] $ExcludeResourceGroupNames = ($ExcludeResourceGroupNames -Split ',').Trim().Replace('"', '').Replace("'", '')
    Write-Output 'ExcludeResourceGroupNames: ' $($ExcludeResourceGroupNames -join ',')
}

if ($ExcludeVmNames) {
    [string[]] $ExcludeVmNames = ($ExcludeVmNames -Split ',').Trim().Replace('"', '').Replace("'", '')
    Write-Output 'ExcludeVmNames: ' $($ExcludeVmNames -join ',')
}

if ($ExcludeTags) {
    Write-Output "ExcludeTags: $ExcludeTags"
...
if ($ExcludeCreatedAfter) {
    ...
    Write-Output "ExcludeCreatedAfter: $ExcludeCreatedAfter"
}
...
if ($ExcludeStoppedAfter) {
    ...
    Write-Output "ExcludeStoppedAfter: $ExcludeStoppedAfter"
}
...
try {
    ...
    if (-not $context) {
        ...
        Write-Output "New connection as $($context.Account.Id)"
    }
    ...
    Write-Output "Subcription set to: $($context.Subscription.Id) established"
}
catch {
    Write-Output "Unable to Connect-AzAccount to Subscription ($SubscriptionId) using Managed Identity"
    throw $_
}
...
if (($IncludeResourceGroupNames -and $IncludeResourceGroupNames.count -eq 1) `
    ...
    Write-Output '==================== GETTING SINGLE VM ===================='
    $vms = Get-AzVm -Status -ResourceGroupName $IncludeResourceGroupNames[0] -Name $IncludeVmNames[0]
    if (-not $vms) {
        Write-Output 'No VMs found matching the specified criteria.'
        return
    }
    ...
    try {
        ...
        Write-Output 'Selecting Virtual Machines...'
        Write-Output $cmd

        $vms = Invoke-Expression $cmd
        Write-Output 'Selection done.'
        if (-not $vms) {
            Write-Output 'No VMs found matching the specified criteria.'
            return
        }
...
Write-Output "VMs to $($Action): $($vms.Name -join ', ')"

# if not array, only one VM found
if ($vms -isnot [array]) {
    Write-Output "==================== EXECUTING $action ===================="
...
foreach ($vm in $vms) {
    ...
    if ($ExcludeStoppedAfter) {
        ...
        if ($stopActivities) {
            Write-Output "$($vm.ResourceGroupName)/$($vm.Name) no action required, stopped on $($stopActivities[0].EventTimestamp)."
            continue
        }
    }
    ...
    if ($Action -eq 'Start') {
        if ($vm.PowerState -eq 'VM Running') {
            Write-Output "$($vm.ResourceGroupName)/$($vm.Name) no action required,$($vm.Powerstate)."
            continue
        }
